...
if test -e MediaConch-Policy/Project/GNU/CLI/configure; then
    cd MediaConch-Policy/Project/GNU/CLI/
    test -e Makefile && rm Makefile
    chmod +x configure
    if [ "$OS" = "mac" ]; then
    ...
    if test ! -e Makefile; then
        echo "Problem while configuring MediaConch-Policy (CLI)"
        exit
    fi
else
...
if test -e MediaConch-Implementation/Project/GNU/CLI/configure; then
    cd MediaConch-Implementation/Project/GNU/CLI/
    test -e Makefile && rm Makefile
    chmod +x configure
    if [ "$OS" = "mac" ]; then
        ./configure $MacOptions --enable-staticlibs $*
    ...
    if test ! -e Makefile; then
        echo "Problem while configuring MediaConch-Implementation (CLI)"
        exit
    fi
else
    echo MediaConch-Policy directory is not found
...
make clean
Parallel_Make
if test -e mediaconch-policy; then
    echo "MediaConch-Policy (CLI) compiled"
else
    echo "Problem while compiling MediaConch-Policy (CLI)"
    exit
fi
cd $Home
...
make clean
Parallel_Make
if test -e mediaconch-implementation; then
    echo "MediaConch-Implementation (CLI) compiled"
else
    echo "Problem while compiling MediaConch-Implementation (CLI)"
    exit
fi
cd $Home
...
echo "MediaConch-Policy executable is MediaConch-Policy/Project/GNU/CLI/mediaconch-policy"
echo "For installing, cd MediaConch-Policy/Project/GNU/CLI && make install"
echo "MediaConch-implementation executable is MediaConch-Implementation/Project/GNU/CLI/mediaconch-implementation"
echo "For installing, cd MediaConch-Implementation/Project/GNU/CLI && make install"
